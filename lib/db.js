...
export async function saveProfile(userId, profile, profileData) {
  ...
  console.log("[DB] saveProfile — resume:", pd.resumeFileName, "| resumeAt:", pd.resumeUploadedAt, "| isUpdate:", isUpdate);
  return q(isUpdate ? `user_profile?user_id=eq.${userId}` : "user_profile", {
    ...
    body: JSON.stringify({
      ...
      is_resume_uploaded: !!pd.resumeFileName,
      resume_file_name: pd.resumeFileName || null,
      resume_uploaded_at: pd.resumeUploadedAt || null,
    }),
  });
}
...
// ── Delete application ────────────────────────────────────────────────────────
export async function deleteApplication(jobApplicationId) {
  if (!jobApplicationId) return;
  // Soft-delete related records first
  await q(`job_app_missing_skill?job_application_id=eq.${jobApplicationId}`, {
    method: "PATCH",
    body: JSON.stringify({ is_deleted: true, modified_time: new Date().toISOString() }),
  }).catch(() => {});
  // Delete the application itself
  return q(`job_application?job_application_id=eq.${jobApplicationId}`, {
    method: "DELETE",
  });
}
...
export async function getSubscriptionStatus(userId) {
  const rows = await q(
    `user_subscription?user_id=eq.${userId}&is_deleted=eq.false&is_active=eq.true&order=created_time.desc&limit=1&select=payment_status,current_period_end,trial_ends_at,plan_id,cancel_at_period_end,stripe_subscription_id,subscription_plan(plan_name)`
  );
  return rows?.[0] || null;
}

// ── AI Query Log ──────────────────────────────────────────────────────────────
export async function saveAIQueryLog(userId, entry) {
  return q("ai_query_log", {
    method: "POST",
    body: JSON.stringify({
      user_id: userId,
      query_type: entry.queryType || "form_answer",
      prompt: (entry.prompt || "").slice(0, 5000),
      answer: (entry.answer || "").slice(0, 5000),
      question: (entry.question || "").slice(0, 2000),
      field_type: entry.fieldType || "text",
...
export async function getAIQueryLogs(userId, limit = 500) {
  return await q(
    `ai_query_log?user_id=eq.${userId}&order=created_time.desc&limit=${limit}`
  ) || [];
}

export async function getAllApplicationAnswers(appIds) {
  if (!appIds?.length) return [];
  return await q(
    `job_application_answer?job_application_id=in.(${appIds.join(",")})&is_deleted=eq.false&order=created_time.asc&select=job_application_id,question,answer,question_type,created_time`
  ) || [];
}
